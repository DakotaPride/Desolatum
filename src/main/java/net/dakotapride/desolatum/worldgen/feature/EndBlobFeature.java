...
public class EndBlobFeature extends Feature<BlockStateConfiguration> {
    public EndBlobFeature(Codec<BlockStateConfiguration> p_65248_) {
        super(p_65248_);
    }
    ...
    public boolean place(FeaturePlaceContext<BlockStateConfiguration> p_159471_) {
        ...
        for (blockstateconfiguration = p_159471_.config(); blockpos.getY() > worldgenlevel.getMinBuildHeight() + 3; blockpos = blockpos.below()) {
            if (!worldgenlevel.isEmptyBlock(blockpos.below())) {
                ...
                if (DesolatumUtils.isEndStoneAdjacent(blockstate)) {
                    break;
                }
            }
        }
