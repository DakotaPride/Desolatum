package net.dakotapride.desolatum.worldgen.feature;
...
public class ViriditeSpikeFeature extends Feature<NoneFeatureConfiguration> {
    public ViriditeSpikeFeature(Codec<NoneFeatureConfiguration> p_66003_) {
        super(p_66003_);
    }
    ...
    public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> p_159882_) {
        ...
        if (!worldgenlevel.getBlockState(blockpos).is(DesolatumBlocks.CHLORITE.get())) {
            return false;
        } else {
                ...
                for (int i1 = -l; i1 <= l; i1++) {
                    ...
                    for (int j1 = -l; j1 <= l; j1++) {
                        ...
                        if ((i1 == 0 && j1 == 0 || !(f1 * f1 + f2 * f2 > f * f))
                            ...
                            if (blockstate.isAir() || isDirt(blockstate) || blockstate.is(DesolatumBlocks.CHLORITE.get())) {
                                this.setBlock(worldgenlevel, blockpos.offset(i1, k, j1), DesolatumBlocks.VIRIDITE_BLOCK.get().defaultBlockState());
                            }
                ...
                for (int i2 = -k1; i2 <= k1; i2++) {
                    ...
                    while (blockpos1.getY() > 50) {
                        ...
                        if (!blockstate1.isAir()
                                && !isDirt(blockstate1)
                                && !blockstate1.is(DesolatumBlocks.CHLORITE.get())
                                && !blockstate1.is(DesolatumBlocks.VIRIDITE_BLOCK.get())) {
                            break;
                        }

                        this.setBlock(worldgenlevel, blockpos1, DesolatumBlocks.VIRIDITE_BLOCK.get().defaultBlockState());
